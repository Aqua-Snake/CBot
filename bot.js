...
async function CBot () {
    ...
    clh.exc_pl = exc_fn
    clh.pay = ddd
    const CBotCN = new WAConnection();
    const Session = new StringSession();
    CBotCN.version = [2, 2126, 14]
    CBotCN.setMaxListeners(0);
    var proxyAgent_var = ''
    if (config.PROXY.includes('https') || config.PROXY.includes('http')) {
      CBotCN.connectOptions.agent = ProxyAgent (config.PROXY)
    }
    setInterval(async () => { 
        var getGMTh = new Date().getHours()
        var getGMTm = new Date().getMinutes()
        var ann_msg = await CBotStack.daily_announcement(config.LANG)
        var ann = await CBotStack.ann()
        while (getGMTh == 19 && getGMTm == 1) {
            ...
            if (ann.video.includes('http') || ann.video.includes('https')) {
                ...
                yt.on('end', async () => {
                    return await CBotCN.sendMessage(CBotCN.user.jid,fs.readFileSync('./' + VID + '.mp4'), MessageType.video, {caption: ilan + ann_msg.replace('{user}', CBotCN.user.name).replace('{wa_version}', CBotCN.user.phone.wa_version).replace('{version}', config.VERSION).replace('{os_version}', CBotCN.user.phone.os_version).replace('{device_model}', CBotCN.user.phone.device_model).replace('{device_brand}', CBotCN.user.phone.device_manufacturer), mimetype: Mimetype.mp4});
                });
            } else {
                if (ann.image.includes('http') || ann.image.includes('https')) {
                    var imagegen = await axios.get(ann.image, { responseType: 'arraybuffer'})
                    return await CBotCN.sendMessage(CBotCN.user.jid, Buffer.from(imagegen.data), MessageType.image, { caption: ilan + ann_msg.replace('{user}', CBotCN.user.name).replace('{wa_version}', CBotCN.user.phone.wa_version).replace('{version}', config.VERSION).replace('{os_version}', CBotCN.user.phone.os_version).replace('{device_model}', CBotCN.user.phone.device_model).replace('{device_brand}', CBotCN.user.phone.device_manufacturer)})
                } else {
                    return await CBotCN.sendMessage(CBotCN.user.jid, ilan + ann_msg.replace('{user}', CBotCN.user.name).replace('{wa_version}', CBotCN.user.phone.wa_version).replace('{version}', config.VERSION).replace('{os_version}', CBotCN.user.phone.os_version).replace('{device_model}', CBotCN.user.phone.device_model).replace('{device_brand}', CBotCN.user.phone.device_manufacturer), MessageType.text)
                }
            }
        }
    }, 50000);
    ...
    setInterval(async () => { 
        if (config.AUTOBIO == 'true') {
            var timezone_bio = await CBotStack.timezone(CBotCN.user.jid)
            var date_bio = await CBotStack.datebio(config.LANG)
            const biography = '📅 ' + date_bio + '\n⌚ ' + timezone_bio
            await CBotCN.setStatus(biography)
        }
    }, 7890);
    var shs1 = ''
    var shl2 = ''
    ...
    var ttq7 = ''
    var ttl8 = ''
    await axios.get('https://gist.githubusercontent.com/Aqua-Snake/636a615c86605e9381a49e00d06a8382/raw/').then(async (insult) => {
    ...
    CBotCN.logger.level = logger_levels
    var nodb;
    if (StrSes_Db.length < 1) {
        nodb = true;
        CBotCN.loadAuthInfo(Session.deCrypt(config.SESSION)); 
    } else {
        CBotCN.loadAuthInfo(Session.deCrypt(StrSes_Db[0].dataValues.value));
    }
    CBotCN.on('open', async () => {
        console.log(
            chalk.blueBright.italic('✅ Login Information Updated!')
        );
        const authInfo = CBotCN.base64EncodedAuthInfo();
        if (StrSes_Db.length < 1) {
    ...
    CBotCN.on('connecting', async () => {
        console.log(`${chalk.green.bold('Whats')}${chalk.blue.bold('Asena')}
${chalk.white.bold('Version:')} ${chalk.red.bold(config.VERSION)}

${chalk.blue.italic('ℹ️ Connecting to WhatsApp... Please Wait.')}`);
    });
    CBotCN.on('credentials-updated', async () => {
        console.log(
        ...
        console.log(chalk.bgGreen(' CBot' + afwhasena));
        await new Promise(r => setTimeout(r, 500));
        let EVA_ACTİON = config.LANG == 'TR' || config.LANG == 'AZ' ? '*CBot Chatbot Olarak Çalışıyor!* 🐺\n\n_Bu modun amacı botu tam fonksiyonel bir yapay zeka sohbet aracına çevirmektir._\n_Normal moda dönmek için_ *.fulleva off* _komutunu kullanabilirsiniz._\n\n*CBot Kullandığın İçin Teşekkürler 💌*\n    *- Eva*' : '*Cyber Army Bot Working as a Chatbot! 🐺*\n\n_The purpose of this mod is to turn the bot into a fully functional AI chatbot._\n_You can use the_ *.fulleva off* _command to return to normal mode._\n\n*Thanks For Using Cyber Army Bot 💌*\n    *- Eva*'
        if (CBotCN.user.jid == one || CBotCN.user.jid == two || CBotCN.user.jid == three || CBotCN.user.jid == four || CBotCN.user.jid == five || CBotCN.user.jid == six || CBotCN.user.jid == seven || CBotCN.user.jid == eight) {
            await CBotCN.sendMessage(CBotCN.user.jid,nw, MessageType.text), console.log(nw), await new Promise(r => setTimeout(r, 1000))
            await heroku.get(baseURI + '/formation').then(async (formation) => { 
                forID = formation[0].id; 
                await heroku.patch(baseURI + '/formation/' + forID, { 
                    body: { 
        ...
        if (config.FULLEVA == 'true') {
            var eva_msg = await CBotStack.eva_if(config.LANG)
            await CBotCN.sendMessage(CBotCN.user.jid, eva_msg, MessageType.text)
        }
        else {
            var af_start = await CBotStack.work_type(config.WORKTYPE, config.LANG)
            await CBotCN.sendMessage(CBotCN.user.jid, af_start, MessageType.text)
        }
        await git.fetch();
        var commits = await git.log([config.BRANCH + '..origin/' + config.BRANCH]);
        if (commits.total === 0) {
            await CBotCN.sendMessage(
                CBotCN.user.jid,
            ...
            var up_ch = await CBotStack.update(config.LANG)
            await CBotCN.sendMessage(CBotCN.user.jid, up_ch, MessageType.text)
        }
    })
    CBotCN.on('message-new', async msg => {
       
        if (msg.key && msg.key.remoteJid == 'status@broadcast') return;
        if (config.NO_ONLINE) {
            await CBotCN.updatePresence(msg.key.remoteJid, Presence.unavailable);
        }
        // ==================== Greetings ====================
        if (msg.messageStubType === 32 || msg.messageStubType === 28) {
            ...
            if (gb !== false) {
                if (gb.message.includes('{gpp}')) {
                    var ppUrl = await CBotCN.getProfilePicture(msg.key.remoteJid) 
                    var nwjson = await CBotCN.groupMetadata(msg.key.remoteJid)
                    const resim = await axios.get(ppUrl, {responseType: 'arraybuffer'})
                    await CBotCN.sendMessage(msg.key.remoteJid, Buffer.from(resim.data), MessageType.image, { caption: gb.message.replace('{gpp}', '').replace('{botowner}', CBotCN.user.name).replace('{gname}', nwjson.subject).replace('{gowner}', nwjson.owner).replace('{gdesc}', nwjson.desc) });
                } else {
                    var nwjson = await CBotCN.groupMetadata(msg.key.remoteJid)
                    await CBotCN.sendMessage(msg.key.remoteJid, gb.message.replace('{gname}', nwjson.subject).replace('{gowner}', nwjson.owner).replace('{gdesc}', nwjson.desc).replace('{botowner}', CBotCN.user.name), MessageType.text);
                }
            }
            return;
        ...
        events.commands.map(
            async (command) =>  {
                ...
                if ((command.on !== undefined && (command.on === 'image' || command.on === 'photo')
                    ...
                    let sendMsg = false;
                    var chat = CBotCN.chats.get(msg.key.remoteJid)
                    ...
                    if (sendMsg) {
                        if (config.SEND_READ && command.on === undefined) {
                            await CBotCN.chatRead(msg.key.remoteJid);
                        }
                        var match = text_msg.match(command.pattern);
                        if (command.on !== undefined && (command.on === 'image' || command.on === 'photo' )
                        && msg.message.imageMessage !== null) {
                            whats = new Image(CBotCN, msg);
                        } else if (command.on !== undefined && (command.on === 'video')
                        && msg.message.videoMessage !== null) {
                            whats = new Video(CBotCN, msg);
                        } else {
                            whats = new Message(CBotCN, msg);
                        }
                        if (msg.key.fromMe && command.deleteCommand) {
                            var wrs = CBotCN.user.phone.wa_version.split('.')[2]
                        ...
                        try {
                            ...
                            var error_report = await CBotStack.error(config.LANG)
                            await CBotCN.sendMessage(CBotCN.user.jid, error_report.replace('{real_error}', error), MessageType.text, {detectLinks: false})

                            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                                if (error.message.includes('URL')) {
                                    return await CBotCN.sendMessage(CBotCN.user.jid, '*⚕️ HATA ÇÖZÜMLEME [Cyber Army Bot] ⚕️*' + 
                                        '\n========== ```Hata Okundu!``` ==========' +
                                    ...
                                    return await CBotCN.sendMessage(CBotCN.user.jid, '*🙇🏻 Maalesef Bu Hatayı Okuyamadım! 🙇🏻*' +
                                ...
                                if (error.message.includes('URL')) {
                                    return await CBotCN.sendMessage(CBotCN.user.jid, '*⚕️ ERROR ANALYSIS [Cyber Army Bot] ⚕️*' + 
                                        '\n========== ```Error Resolved!``` ==========' +
                                    ...
                                    return await CBotCN.sendMessage(CBotCN.user.jid, '*🙇🏻 Sorry, I Couldnt Read This Error! 🙇🏻*' +
