/* Copyright (C) 2021 Cyber Bot.

Licensed under the  GPL-3.0 License;
you may not use this file except in compliance with the License.

Cyber Bot - Aqua Snake
*/

const CBot = require('../events');
const Config = require('../config');
const {MessageType, MessageOptions, Mimetype} = require('@adiwajshing/baileys');
const fs = require('fs');
const os = require('os');
const qatxtpro = require('CBot-textlogo-1.0.3'); 
const axios = require('axios');
const request = require('request');
var code = { cd: 'L3Jvb3QvUXVlZW5BbWRpLw==', pay: '' }    
var bufcode = Buffer.from(code.cd, 'base64')
var utfbuff = bufcode.toString('utf-8')
code.pay = utfbuff
if (os.userInfo().homedir !== code.pay) return;

// LIST
const Language = require('../language');
const Lang = Language.getString('effects');
let LOL = Config.WORKTYPE == 'public' ? false : true

CBot.applyCMD({pattern: 'demon ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/create-green-horror-style-text-effect-online-1036.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/CBot/demon2.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/CBot/demon2.jpg'), MessageType.image, { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


CBot.applyCMD({pattern: 'rust ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/create-a-metallic-text-effect-free-online-1041.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/CBot/rust.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/CBot/rust.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


CBot.applyCMD({pattern: 'transformer ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/create-a-transformer-text-effect-online-1035.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/CBot/transformer.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/CBot/transformer.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


CBot.applyCMD({pattern: 'fiction ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/create-science-fiction-text-effect-online-free-1038.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/CBot/fiction.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/CBot/fiction.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


CBot.applyCMD({pattern: 'berry ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/create-berry-text-effect-online-free-1033.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/CBot/berry.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/CBot/berry.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


CBot.applyCMD({pattern: 'thunder ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/online-thunder-text-effect-generator-1031.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/CBot/thunder.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/CBot/thunder.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


CBot.applyCMD({pattern: 'magma ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/create-a-magma-hot-text-effect-online-1030.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/CBot/magma.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/CBot/magma.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


CBot.applyCMD({pattern: 'stone ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/3d-stone-cracked-cool-text-effect-1029.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/CBot/stone.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/CBot/stone.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


CBot.applyCMD({pattern: 'glitch ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    var topText, bottomText; 
    if (match[1].includes(';')) {
        var split = match[1].split(';');
        topText = split[0];
        bottomText = split[1];
    } else {
        topText = match[1];
        bottomText = 'ㅤ';
    }
    qatxtpro.textpro("https://textpro.me/create-glitch-text-effect-style-tik-tok-983.html",
        [`${topText}`, `${bottomText}`]
        ).then(async (data) => { 
          try { 
              var download = async(uri, filename, callback) => {
                  await request.head(uri, async(err, res, body) => {    
                      await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                  });
              };
              await download(`${data}`, '/root/CBot/glitch.jpg', async() => {                          
                  await message.client.sendMessage(message.jid,fs.readFileSync('/root/CBot/glitch.jpg'), MessageType.image, { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
              })
          } catch(err) { 
              console.log(err)
          } 
    });
}));

CBot.applyCMD({pattern: '2glitch ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    var topText, bottomText; 
    if (match[1].includes(';')) {
        var split = match[1].split(';');
        topText = split[0];
        bottomText = split[1];
    } else {
        topText = match[1];
        bottomText = 'ㅤ';
    }
    qatxtpro.textpro("https://textpro.me/create-a-glitch-text-effect-online-free-1026.html",
        [`${topText}`, `${bottomText}`]
        ).then(async (data) => { 
          try { 
              var download = async(uri, filename, callback) => {
                  await request.head(uri, async(err, res, body) => {    
                      await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                  });
              };
              await download(`${data}`, '/root/CBot/t2gl.jpg', async() => {                          
                  await message.client.sendMessage(message.jid,fs.readFileSync('/root/CBot/t2gl.jpg'), MessageType.image, { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
              })
          } catch(err) { 
              console.log(err)
          } 
    });
}));


CBot.applyCMD({pattern: 'harry ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/create-harry-potter-text-effect-online-1025.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/CBot/harry.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/CBot/harry.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


CBot.applyCMD({pattern: 'cracked ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/create-embossed-text-effect-on-cracked-surface-1024.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/CBot/cracked.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/CBot/cracked.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


CBot.applyCMD({pattern: 'glass ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/broken-glass-text-effect-free-online-1023.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/CBot/glass.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/CBot/glass.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


CBot.applyCMD({pattern: 'glossy ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/create-a-3d-glossy-metal-text-effect-1019.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/CBot/glossy.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/CBot/glossy.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


CBot.applyCMD({pattern: 'beach ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/create-3d-realistic-text-effect-on-the-beach-online-1018.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/CBot/beach.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/CBot/beach.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


CBot.applyCMD({pattern: 'watercolour ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/create-a-free-online-watercolor-text-effect-1017.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/CBot/watercolour.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/CBot/watercolour.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


CBot.applyCMD({pattern: 'devil ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/create-neon-devil-wings-text-effect-online-free-1014.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/CBot/devil.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/CBot/devil.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


CBot.applyCMD({pattern: 'underwater ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/3d-underwater-text-effect-generator-online-1013.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/CBot/underwater.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/CBot/underwater.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


CBot.applyCMD({pattern: 'bear ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/online-black-and-white-bear-mascot-logo-creation-1012.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/CBot/bear.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/CBot/bear.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


CBot.applyCMD({pattern: 'graffiti ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/create-wonderful-graffiti-art-text-effect-1011.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/CBot/1graffiti.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/CBot/1graffiti.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


CBot.applyCMD({pattern: 'xmas ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/create-a-christmas-holiday-snow-text-effect-1007.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/CBot/xmas.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/CBot/xmas.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


CBot.applyCMD({pattern: 'future ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/create-a-futuristic-technology-neon-light-text-effect-1006.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/CBot/future.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/CBot/future.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


CBot.applyCMD({pattern: 'cloud ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/create-a-cloud-text-effect-on-the-sky-online-1004.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/CBot/cloud.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/CBot/cloud.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


CBot.applyCMD({pattern: 'luxury ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/3d-luxury-gold-text-effect-online-1003.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/CBot/luxury.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/CBot/luxury.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


CBot.applyCMD({pattern: 'leaves ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/natural-leaves-text-effect-931.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/CBot/leaves.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/CBot/leaves.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


CBot.applyCMD({pattern: 'gneon ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/green-neon-text-effect-874.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/CBot/gneon.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/CBot/gneon.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


CBot.applyCMD({pattern: 'holo ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/holographic-3d-text-effect-975.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/CBot/holo.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/CBot/holo.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));


CBot.applyCMD({pattern: 'sand ?(.*)', fromMe: LOL,  deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        
    qatxtpro.textpro("https://textpro.me/write-in-sand-summer-beach-free-online-991.html", `${match[1]}` ).then(async (data) => { 
        try {
            var download = async(uri, filename, callback) => {
                await request.head(uri, async(err, res, body) => {
                    await request(uri).pipe(fs.createWriteStream(filename)).on('close', callback);
                });
            };
            await download(`${data}`, '/root/CBot/sand.jpg', async() => {
                await message.client.sendMessage(message.jid,fs.readFileSync('/root/CBot/sand.jpg'), MessageType.image,  { caption: Config.CAP, quoted: message.data, thumbnail: thumb })
            })
        } catch(err) { 
            console.log(err)
        }
    });
}));




const thumb = ""