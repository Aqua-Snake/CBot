...
const WhatsCBotStack = require('cbot-npm');
...
CBot.addCommand({pattern: 'animepic ?(.*)', fromMe: wk, desc: pic, usage: 'animepic normal // animepic nsfw'}, (async (message, match) => {
  if (match[1] == 'normal') {
    var image_link = await CBotStack.anime_wall('normal')
    var image_buffer = await axios.get(image_link, { responseType: 'arraybuffer' })
    await message.sendMessage(Buffer.from(image_buffer.data), MessageType.image, { caption: 'Made by Cyber Army Bot' })
  } else if (match[1] == 'nsfw') {
    var image_link_nsfw = await CBotStack.anime_wall('nsfw')
    var image_buffer_nsfw = await axios.get(image_link_nsfw, { responseType: 'arraybuffer' })
    await message.sendMessage(Buffer.from(image_buffer_nsfw.data), MessageType.image, { caption: 'Made by Cyber Army Bot' })
  } else {
    return await message.client.sendMessage(message.jid,wr_usage,MessageType.text)
  }
}));
CBot.addCommand({pattern: 'animegif ?(.*)', fromMe: wk, desc: giff, usage: 'animegif normal // animegif nsfw'}, (async (message, match) => {
  if (match[1] == 'normal') {
    var gif_link = await CBotStack.anime_gif('normal')
    var gif_buffer = await axios.get(gif_link, { responseType: 'arraybuffer' })
    await message.sendMessage(Buffer.from(gif_buffer.data), MessageType.video, { caption: 'Made by Cyber Army Bot', mimetype: Mimetype.gif })
  } else if (match[1] == 'nsfw') {
    var gif_link_nsfw = await CBotStack.anime_gif('nsfw')
    var gif_buffer_nsfw = await axios.get(gif_link_nsfw, { responseType: 'arraybuffer' })
    await message.sendMessage(Buffer.from(gif_buffer_nsfw.data), MessageType.video, { caption: 'Made by Cyber Army Bot', mimetype: Mimetype.gif })
  } else {
    return await message.client.sendMessage(message.jid,wr_usage,MessageType.text)
  }
}));
