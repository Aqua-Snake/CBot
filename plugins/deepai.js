...
const deepai = require('deepai'); // In local, you can use it in a customizable way by creating deepmain.js. Doesn't Work on Web Servers!!
...
if (Config.WORKTYPE == 'private') {
    ...
    CBot.addCommand({pattern: 'colorai$', fromMe: true, deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {    
        ...
        ffmpeg(location)
            ...
            .on('end', async () => {
                ...
                await message.sendMessage(Buffer.from(respoimage.data), MessageType.image, { mimetype: Mimetype.jpg, caption: 'Made by CBot'})
            });
    ...
    Asena.addCommand({pattern: 'moodai ?(.*)', fromMe: true, deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        ...
        var msgdata = await CBotStack.mood(match[1], Config.DEEPAI)
    ...
    Asena.addCommand({pattern: 'ttiai ?(.*)', fromMe: true, deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        if (!Config.DEEPAI) return await message.sendMessage(noAPI);
        if (match[1] === '') return await message.sendMessage(Lang.TEXT);
        var msg_tt = await CBotStack.tti(match[1], Config.DEEPAI)
    ...
    Asena.addCommand({pattern: 'textai ?(.*)', fromMe: true, deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
        if (!Config.DEEPAI) return await message.sendMessage(noAPI);
        if (match[1] === '') return await message.sendMessage(Lang.TEXT);
        var text_ai = await CBotStack.text(match[1], Config.DEEPAI)
    ...
    Asena.addCommand({pattern: 'faceai$', fromMe: false, deleteCommand: false, dontAddCommandList: true }, (async (message, match) => {
        var image_an = await CBotStack.face()
        var webimage = await axios.get(image_an, {responseType: 'arraybuffer'})
        await message.sendMessage(Buffer.from(webimage.data), MessageType.image, { mimetype: Mimetype.jpg, caption: 'Made by CBot'})
    }));
