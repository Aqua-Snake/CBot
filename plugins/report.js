...
if (Config.WORKTYPE == 'private') {
    CBot.applyCMD({pattern: 'report ?(.*)', fromMe: true, desc: Lang.REPORT,  deleteCommand: false}, (async (message, match) => {
    ...
    CBot.applyCMD({pattern: 'report ?(.*)', fromMe: false, desc: Lang.REPORT}, (async (message, match) => {
        if (match[1] == '' && message.reply_message) {
            let grp = await message.client.groupMetadata(message.jid);
            var jids = [];
            msg = '';
