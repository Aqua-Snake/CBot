...
CBot.addCommand({on: 'text', fromMe: wk, dontAddCommandList: true, deleteCommand: false}, (async (message, match) => {
    if (message.message.startsWith('Eva') && eva_functionality !== 'true') {        
        ...
        let acc = os.userInfo().homedir.split('C')[1].split('Bot/')[0] == 'CBot' ? '7d57838203msh0c5cf65c90a7231p13b461jsn77c8cfa55871' : '7d57838203msh0c582jak19865261js1229n77c8cfa55871'
        let aitalk_mode = ''
        ...
        var ainame = os.userInfo().homedir.split('C')[1].split('Bot/')[0]
        if (ainame !== 'CBot') return;
        ...
        var payload_client = await CBotStack.get_eva_ai(encodeURIComponent(trmsg), 'Eva', 'Aqua-Snake', 'CBot', unique_ident)
        var fins = ''                           
        if (conf.LANG !== 'EN') {
    ...
    try {
        ...
        convertToWav(file)
            .on('end', async () => {
                ...
                var ainame = os.userInfo().homedir.split('C')[1].split('Bot/')[0]
                if (ainame !== 'CBot') return;
                var payload_client = await CBotStack.get_eva_ai(encodeURIComponent(ssc), 'Eva', 'Aqua-Snake', 'CBot', unique_ident)
                var fins = ''                           
                if (conf.LANG !== 'EN') {
                    ceviri = await translatte(payload_client.result, {from: 'auto', to: conf.LANG});
                    if ('text' in ceviri) {
