...
Asena.addCommand({pattern: 'speedtest ?(.*)', fromMe: true, desc: Lang.SPEEDTEST_DESC, usage: 'speedtest user // speedtest server'}, (async (message, match) => {
    if (match[1] == 'user' || match[1] == 'User') {
        ...
        // Preliminary Message
        var transition_message = await CBotStack.speedtest_once(Config.LANG)
        ...
        // Speedtest Modules
        var user_download = await CBotStack.speedtest_user()
        var user_upload = await CBotStack.uploadtest_user()
        var auth_message = await CBotStack.speedtest_message(Config.LANG)
        ...
        // Speedtest Modules
        var server_download = await CBotStack.speedtest_server()
        var server_upload = await CBotStack.uploadtest_server()
        var auth_message = await CBotStack.speedtest_message(Config.LANG)
        var act_ping = end - start
